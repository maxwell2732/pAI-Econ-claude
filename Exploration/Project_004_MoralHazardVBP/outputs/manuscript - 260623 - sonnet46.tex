...
\item \textbf{Participation threshold (Proposition~\ref{prop:threshold}).} A unique originator exit threshold $\bar{p}$ exists under standard regularity conditions. Below $\bar{p}$, the originator cannot cover its fixed costs and exits; above it, the originator participates.
...
Under Assumptions~\ref{ass:demand}--\ref{ass:singlepeaked}, there exists a unique $\pbar \in (c_o, p_O^{\max})$ such that $\Pi_O(p) \geq 0$ if and only if $p \geq \pbar$. The threshold $\pbar$ is implicitly defined by
\begin{equation}
...
\begin{proof}
$\Pi_O(c_o) = -F < 0$ by Assumption~\ref{ass:fixedcost}. By Assumption~\ref{ass:viable}, $\Pi_O(p_O^{\max}) > 0$. Since $\Pi_O$ is continuous (from Assumption~\ref{ass:demand}), the Intermediate Value Theorem gives at least one root $\pbar \in (c_o, p_O^{\max})$. Assumption~\ref{ass:singlepeaked} implies $\Pi_O$ is strictly increasing on $(c_o, p_O^{\max})$, so the root is unique and $\pbar$ is the lower zero-profit price. Monotonicity in $N$ and $F$ follows from implicit differentiation of \eqref{eq:threshold}.
\end{proof}
